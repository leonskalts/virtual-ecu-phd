...
OUT=ROOT/'results/cross_layer_safety_v7_1_dev/validation/gui_candidate2';OUT.mkdir(parents=True,exist_ok=True)
...
from virtual_ecu.clo_dsf_candidate2_gui import LABEL,C1_LABEL,read_evidence,Candidate2EvidenceView
...
try:
 ...
 check(bool(panel.candidate2_evidence.rows),'Advanced run loads Candidate 2 runtime sidecar')
 check(any(r['detector_state']=='CONFIRMED' and r['estimated_origin']=='TIMING' for r in panel.candidate2_evidence.rows),'Live runtime detects and localizes timing')
 check(not any('true_origin' in k for r in panel.candidate2_evidence.rows for k in r),'Runtime panel excludes evaluation labels')
 panel.candidate2_evidence.selector.current(0);panel.candidate2_evidence.show(0);check(panel.candidate2_evidence.values['detector_state'].get()=='NORMAL','Runtime selector shows initial state')
 # Load an actual selected-validation sensor trace with CONFIRMED + UNKNOWN.
 import csv
 manifest=list(csv.DictReader((ROOT/'results/cross_layer_safety_v7_1_dev/development_split_manifest.csv').open()));spec=next(s for s in manifest if s['partition']=='development-validation' and s['origin']=='SENSOR_CONTROL')
 panel.candidate2_evidence.load(ROOT/f"results/cross_layer_safety_v7_1_dev/traces/{spec['run_id']}.candidate2.csv.gz")
 check(panel.candidate2_evidence.values['detector_state'].get()=='CONFIRMED' and panel.candidate2_evidence.values['estimated_origin'].get()=='UNKNOWN','Confirmed anomaly with UNKNOWN origin is displayed as valid')
 ...
 preview=tk.Toplevel(app);preview.geometry('1100x480');preview.title('Candidate 2 runtime evidence')
 view=Candidate2EvidenceView(preview);view.pack(fill='both',expand=True,padx=8,pady=8)
 view.load(ROOT/f"results/cross_layer_safety_v7_1_dev/traces/{spec['run_id']}.candidate2.csv.gz")
 ...
 app.research_workspace.show('runtime_study');dev=app.research_workspace.candidate2_development;dev.load();app.update();check(len(dev.table.get_children())==16,'Development comparison includes all 16 methods and ablations');check(dev.truth.winfo_manager()=='grid','Evaluation table ground truth is separate')
 check(hasattr(app.research_workspace,'clo_development'),'Original Candidate 1 study panel retained')
 dev.example();wait_for(lambda:bool(dev.runtime.rows));check(any(r['alarm']=='1' for r in dev.runtime.rows),'Research example executes Candidate 2 C path')
 app.page_frames['runtime_study'].canvas.yview_moveto(.2);app.update();capture(app,OUT/'development_comparison.png')
 panel.load_results(ROOT/'results/cross_layer_safety_v7_dev/gui_runtime/single_experiment.csv');check(bool(panel.clo_evidence.rows) and not panel.candidate2_evidence.winfo_manager(),'Frozen Candidate 1 runtime result still loads')
 panel.load_results(ROOT/'results/cross_layer_safety_v1/raw/baseline.csv');check(not panel.candidate2_evidence.winfo_manager(),'Historical CSV hides unavailable Candidate 2 evidence')
