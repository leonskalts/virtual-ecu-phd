...
#define METHODS 10
static const char *const names[METHODS]={"Final CLO-DSF","Candidate 2","Candidate 2 no origin discount","Final no temporal","Candidate 1","Plain DS","Simple OR","Weighted Sum","Hybrid","Timing Monitor"};
static clo_final_t detector,no_temporal;
static clo_final_config_t config;
static c2_state_t candidate2,no_origin;
...
void __wrap_detection_algorithm_step(struct ecu_state *state)
{
 ...
 clo_final_step(&detector,&config,&o);
 ...
 if(comparison) {
  ...
  c2_step(&no_origin,&c2_config,C2_FULL^C2_ORIGIN_RELIABILITY,&o);final_score(2,o.time_ms,&no_origin.output);
  clo_final_config_t no_time=config;no_time.lambda_temporal=0;
  clo_final_step(&no_temporal,&no_time,&o);final_score(3,o.time_ms,&no_temporal.output);
  clo_dsf_step(&c1,&c1_config,CLO_FULL,&o);old_score(4,o.time_ms,&c1);
  clo_dsf_step(&plain,&plain_config,CLO_PLAIN,&o);old_score(5,o.time_ms,&plain);
  double peak=0,ws=0;for(int i=0;i<6;i++){double e=detector.evidence.strength[i];if(e>peak)peak=e;ws+=ws_weights[weighted_choice/6][i]*e;}
  simple_score(6,o.time_ms,peak>=.5);simple_score(7,o.time_ms,ws>=ws_thresholds[weighted_choice%6]);
  ecu_state_t shadow=*state;shadow.detection=hybrid;__real_detection_algorithm_step(&shadow);hybrid=shadow.detection;
  simple_score(8,o.time_ms,hybrid.alarm_active);simple_score(9,o.time_ms,state->timing_monitor.alarm);
 }
 if(primary){state->detection.current_score=(float)detector.output.anomaly_decision_score;state->detection.alarm_active=detector.output.alarm;state->detection.detected=detector.output.alarm_timestamp_ms>=0;state->detection.first_detection_time_ms=detector.output.alarm_timestamp_ms;snprintf(state->detection.runtime_label,sizeof(state->detection.runtime_label),"clo_dsf_final");}
...
int main(int argc,char **argv)
{
 ...
 for(int i=1;i<argc;i++) {
  if(!strcmp(argv[i],"--final-comparison")){comparison=true;enabled=true;continue;}
  if(!strncmp(argv[i],"--final-",8)) {
   const char *key=argv[i];if(++i>=argc){fprintf(stderr,"Missing %s value\n",key);return 1;}
   if(!strcmp(key,"--final-evidence"))trace_path=argv[i];
   else if(!strcmp(key,"--final-observations"))observation_path=argv[i];
   else if(!strcmp(key,"--final-config"))config_path=argv[i];
   else if(!strcmp(key,"--final-c2-config"))c2_path=argv[i];
   else if(!strcmp(key,"--final-metrics"))metrics_path=argv[i];
   else if(!strcmp(key,"--final-evaluation-start")){if(number(argv[i],&evaluation_start))return 1;}
   else if(!strcmp(key,"--final-weighted-choice")){unsigned int n;if(number(argv[i],&n)||n>=12)return 1;weighted_choice=(int)n;ws_set=true;}
   else {fprintf(stderr,"Invalid final option: %s\n",key);return 1;}
   enabled=true;
  } else if(!strcmp(argv[i],"--detector")&&i+1<argc&&!strcmp(argv[i+1],"clo_dsf_final")) {
   primary=enabled=true;argv[write++]=argv[i++];argv[write++]="builtin_ecu";
  } else argv[write++]=argv[i];
 }
 argc=write;argv[write]=NULL;
 if(enabled && (!config_path || final_config_read(config_path,&config))){fprintf(stderr,"A complete valid --final-config is required.\n");return 1;}
 ...
 clo_final_init(&detector);clo_final_init(&no_temporal);c2_init(&candidate2);c2_init(&no_origin);
 clo_config_default(&c1_config);c1_config.conflict_rule=DS_DEMPSTER;plain_config=c2_config.common;clo_dsf_init(&c1);clo_dsf_init(&plain);
 ...
 if(primary&&!trace_path){const char *raw=ECU_DEFAULT_LOG_PATH;if(argc>1){size_t n=strlen(argv[1]);if(n>=4&&!strcmp(argv[1]+n-4,".csv"))raw=argv[1];}int n=snprintf(automatic,sizeof(automatic),"%s.final.csv",raw);if(n<0||(size_t)n>=sizeof(automatic))return 1;trace_path=automatic;}
